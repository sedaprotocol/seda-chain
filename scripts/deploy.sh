...
CONTRACT_DEPLOY_OUTPUT=$(sedad tx wasm store $CONTRACT_PATH --from $DEPLOYER_ADDRESS --gas-prices 0.1seda --gas auto --gas-adjustment 1.3 -y --output json)
CONTRACT_DEPLOY_TX_HASH=$(echo $CONTRACT_DEPLOY_OUTPUT | jq -r .txhash)

sleep 2;

CONTRACT_DEPLOY_TX_OUTPUT=$(sedad query tx $CONTRACT_DEPLOY_TX_HASH --output json)
...
INSTANTIATE_OUTPUT=$(sedad tx wasm instantiate $CONTRACT_DEPLOY_CODE_ID $INITIAL_STATE --admin="$(sedad keys show $DEPLOYER_ADDRESS -a)" --from $DEPLOYER_ADDRESS --label "local0.1.0" -y --output json) 
