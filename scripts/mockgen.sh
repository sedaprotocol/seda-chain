...
# Generate mocks
wasmd_dir=$(go list -f '{{.Dir}}' github.com/CosmWasm/wasmd/x/wasm/types)
$mockgen_cmd -source=$wasmd_dir/exported_keepers.go -package testutil -destination=x/wasm-storage/keeper/testutil/wasm_keepers_mock.go

aws_sdk_dir=$(go list -f '{{.Dir}}' github.com/aws/aws-sdk-go/service/sqs/sqsiface)
$mockgen_cmd -source=$aws_sdk_dir/interface.go -package testutil -destination=plugins/indexing/pluginaws/testutil/sqs_client_mock.go
...
$mockgen_cmd -source=x/core/types/expected_keepers.go -package testutil -destination=x/core/keeper/testutil/expected_keepers_mock.go
...
$mockgen_cmd -source=$GOPATH/pkg/mod/github.com/aws/aws-sdk-go@v1.55.5/service/sns/snsiface/interface.go -package testutil -destination=plugins/indexing/pluginaws/testutil/sns_client_mock.go
