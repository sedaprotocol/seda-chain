...
func (m *MsgRegisterFastClient) ValidateBasic() error {
...
func (m *MsgEditFastClient) ValidateBasic() error {
	if _, err := sdk.AccAddressFromBech32(m.OwnerAddress); err != nil {
		return sdkerrors.ErrInvalidAddress.Wrapf("invalid owner address: %s", m.OwnerAddress)
	}
...
func (m *MsgTransferOwnership) ValidateBasic() error {
	...
	_, err := hex.DecodeString(m.FastClientPublicKey)
	if err != nil {
		return sdkerrors.ErrInvalidRequest.Wrapf("invalid hex in pubkey: %s", m.FastClientPublicKey)
	}
...
func (m *MsgRemoveUser) ValidateBasic() error {
	...
	if len(m.FastClientPublicKey) > MaxPublicKeyLength {
		return sdkerrors.ErrInvalidRequest.Wrapf("public key is too long; got: %d, max < %d", len(m.FastClientPublicKey), MaxPublicKeyLength)
	}
...
func (m *MsgExpireUserCredits) ValidateBasic() error {
	if _, err := sdk.AccAddressFromBech32(m.AdminAddress); err != nil {
		return sdkerrors.ErrInvalidAddress.Wrapf("invalid admin address: %s", m.AdminAddress)
	}
